...
function scanImageCountryFn() {
  ...
  const result = imgs.map(img => {
    const m = img.src.match(/\/assets\/([a-z]{2}(?:_[a-z]{2})?)\//i);
    const imgCountry = m ? m[1].toLowerCase() : null;
    const shortSrc = img.src.length > 60 ? '…' + img.src.slice(-57) : img.src;
    return { src: img.src, shortSrc, imgCountry, hasCountryInPath: !!imgCountry };
  });
